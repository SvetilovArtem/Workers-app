...
const Workers = ({ workers }: WorkersProps) => {
  ...
  const perPage = useSelector(
    (state: RootState) => state.workersReducer.perPage
  );
  const currentPage = useSelector(
    (state: RootState) => state.workersReducer.currentPage
  );
  const totalPages = useSelector(
    (state: RootState) => state.workersReducer.totalPages
  );
  ...
  return (
    <>
      <div className={styles.workers}>
        {status
          ...
          : workers.map((worker: Worker) => (
              <WorkerCard dispatch={dispatch} worker={worker} key={worker.id} />
            ))}
      </div>
      <button
      ...
      >
        Показать еще <IconCreator type="arrow" />
      </button>
